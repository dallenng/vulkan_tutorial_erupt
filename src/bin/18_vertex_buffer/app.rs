...
use erupt::vk::{
    ...
    BufferCreateInfoBuilder, BufferUsageFlags, ClearColorValue, ClearValue, ColorComponentFlags,
    ColorSpaceKHR, CommandBuffer, CommandBufferAllocateInfoBuilder, CommandBufferBeginInfoBuilder,
    CommandBufferLevel, CommandPool, CommandPoolCreateInfoBuilder, ComponentMappingBuilder,
    ComponentSwizzle, CompositeAlphaFlagBitsKHR, CullModeFlags, DeviceCreateInfoBuilder,
    DeviceMemory, DeviceQueueCreateInfoBuilder, Extent2D, Extent2DBuilder, FenceCreateFlags,
    ...
    ImageViewType, InstanceCreateInfoBuilder, LogicOp, MemoryAllocateInfoBuilder,
    ...
    PrimitiveTopology, Rect2DBuilder, RenderPass, RenderPassBeginInfoBuilder,
...
impl VulkanApp {
    ...
    fn create_vertex_buffer(
        ...
        let buffer_info = BufferCreateInfoBuilder::new()
            .size((mem::size_of::<Vertex>() * Vertex::VERTICES.len()) as u64)
            .usage(BufferUsageFlags::VERTEX_BUFFER)
            .sharing_mode(SharingMode::EXCLUSIVE);

        let vertex_buffer = unsafe { device.create_buffer(&buffer_info, None) }.result()?;
        ...
        let vertex_buffer_memory = unsafe { device.allocate_memory(&alloc_info, None) }.result()?;
        unsafe { device.bind_buffer_memory(vertex_buffer, vertex_buffer_memory, 0) }.result()?;
