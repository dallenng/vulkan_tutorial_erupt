...
use erupt::vk::{
    ...
    ColorSpaceKHR, CommandBuffer, CommandBufferAllocateInfoBuilder, CommandBufferBeginInfoBuilder,
    ...
    FenceCreateInfoBuilder, Filter, Format, FormatFeatureFlags, Framebuffer,
    FramebufferCreateInfoBuilder, FrontFace, GraphicsPipelineCreateInfoBuilder, Image,
    ImageAspectFlags, ImageBlitBuilder, ImageCreateInfoBuilder, ImageLayout,
    ...
    IndexType, InstanceCreateInfoBuilder, LogicOp, MemoryAllocateInfoBuilder, MemoryPropertyFlags,
    Offset2DBuilder, Offset3DBuilder, PhysicalDevice, PhysicalDeviceFeaturesBuilder, Pipeline,
...
const VERTEX_SHADER_CODE: &[u8] = include_bytes!("../../../shaders/26_shader_depth.vert.spv");
const FRAGMENT_SHADER_CODE: &[u8] = include_bytes!("../../../shaders/26_shader_depth.frag.spv");

const MODEL_BYTES: &[u8] = include_bytes!("../../../models/viking_room.obj");
const TEXTURE_BYTES: &[u8] = include_bytes!("../../../textures/viking_room.png");
...
impl VulkanApp {
    pub fn new(window: &Window) -> VkResult<Self> {
        ...
        let render_pass =
            Self::create_render_pass(&instance, physical_device, &device, swapchain.image_format)?;
        ...
        let (depth_image, depth_image_memory, depth_image_view) =
            Self::create_depth_resources(&instance, physical_device, &device, swapchain.extent)?;
        ...
        let texture_sampler =
            Self::create_texture_sampler(&instance, physical_device, &device, mip_levels)?;
    ...
    fn create_image_views(
        ...
        images
            .iter()
            .map(|image| {
                Self::create_image_view(device, *image, image_format, ImageAspectFlags::COLOR, 1)
            })
            .collect()
    }
    ...
    fn find_supported_format(
        ...
        candidates
            ...
            .copied()
            .ok_or(VkError::NoSupportedFormat)
    }
    ...
    fn create_texture_image(
        ...
        command_pool: CommandPool,
    ) -> VkResult<(Image, DeviceMemory, u32)> {
        ...
        Self::generate_mipmaps(
            ...
            graphics_queue,
            command_pool,
            (texture_image, Format::R8G8B8A8_SRGB),
            (
    ...
    fn create_image(
        ...
        physical_device: PhysicalDevice,
        device: &DeviceLoader,
        (extent, mip_levels): (Extent3D, u32),
        format: Format,
        tiling: ImageTiling,
    ...
    fn generate_mipmaps(
        ...
        (tex_width, tex_height, mip_levels): (i32, i32, u32),
    ) -> VkResult<()> {
        ...
        {
            return Err(VkError::UnsupportedLinearBlitting);
        }
        ...
        for i in 1..mip_levels {
            ...
            unsafe {
                device.cmd_pipeline_barrier(
                    command_buffer,
                    Some(PipelineStageFlags::TRANSFER),
                    Some(PipelineStageFlags::TRANSFER),
                    None,
                    &[],
                    &[],
            ...
            let blit = ImageBlitBuilder::new()
                .src_offsets([
                    *Offset3DBuilder::new().x(0).y(0).z(0),
                    *Offset3DBuilder::new().x(mip_width).y(mip_height).z(1),
                ])
                .src_subresource(
                    *ImageSubresourceLayersBuilder::new()
                        .aspect_mask(ImageAspectFlags::COLOR)
            ...
            unsafe {
                device.cmd_pipeline_barrier(
                    command_buffer,
                    Some(PipelineStageFlags::TRANSFER),
                    Some(PipelineStageFlags::FRAGMENT_SHADER),
                    None,
                    &[],
                    &[],
                    slice::from_ref(&barrier),
    ...
    fn load_model() -> VkResult<(Vec<Vertex>, Vec<u32>)> {
        let (models, _) = tobj::load_obj_buf(
            &mut Cursor::new(MODEL_BYTES),
            &tobj::LoadOptions {
                single_index: true,
                triangulate: true,
        ...
        for tobj::Model { mesh, .. } in models {
            ...
            for &index in &mesh.indices {
                let index = *unique_vertices.entry(index).or_insert_with_key(|index| {
    ...
    fn create_vertex_buffer(
        ...
        let buffer_size = (mem::size_of::<Vertex>() * vertices.len()) as DeviceSize;
    ...
    fn create_index_buffer(
        ...
        let buffer_size = (mem::size_of::<u32>() * indices.len()) as DeviceSize;
    ...
    fn create_command_buffers(
        ...
        command_pool: CommandPool,
        indices: &[u32],
        (buffers, descriptor_sets): (&Buffers, &[DescriptorSet]),
            ...
            .zip(descriptor_sets)
        {
            ...
            unsafe {
                ...
                device.cmd_bind_index_buffer(command_buffer, buffers.index, 0, IndexType::UINT32);
    ...
    fn recreate_swapchain(&mut self, window: &Window) -> VkResult<()> {
        ...
        self.pipeline.render_pass = Self::create_render_pass(
            &self.instance,
        ...
        self.command_buffers = Self::create_command_buffers(
            ...
            self.command_pool,
            &self.indices,
            (&self.buffers, &self.descriptor_sets),
        )?;
    ...
    unsafe fn cleanup_swapchain(&mut self) {
        ...
        self.device.destroy_image(Some(self.depth_image), None);

        self.device.free_memory(Some(self.depth_image_memory), None);
